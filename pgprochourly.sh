...
LOG=$DIR/logs/pgprochourly-$(date +"%Y-%m-%d").log
PG_HOME=/usr/pgsql-9.6
THISHOST=$(hostname -f)
THISHOSTIP=$(hostname -A)
mypidfile=$DIR/pgprochourly.pid
LOCALPORT=6543
...
nohup psql -h localhost -p $LOCALPORT -U postgres -d db_monitoreo -A -t -c "delete FROM public.resultados where estado = 'AZ' fecha < (clock_timestamp() - INTERVAL '7 days') ;" > /dev/null 2>&1 &
